...
#define SUBARU_BASE_TX_MSGS(alt_bus, lkas_msg) \
  {lkas_msg,                     alt_bus,         8, .check_relay = true},  \
...
static inline bool subaru_lkas_dash_state_is_button_press(int cur, int prev) {
  return ((cur == 0) && (prev != 0)) || ((cur != 0) && (prev == 0));
}
...
static void subaru_rx_hook(const CANPacket_t *msg) {
  ...
  if (msg->addr == MSG_SUBARU_ES_LKAS_State) {
